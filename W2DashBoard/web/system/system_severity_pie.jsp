...
<%
	...
	int total = info.getCritical() + info.getMajor() + info.getMinor();
%>
...
$(function () {
    ...
    $('#luna_resolution_pie').highcharts({
    	credits: {
			  /* enabled : false */	   
		text: 'Edit',
		href: 'javascript:window.open("system_severity_edit_open","Edit","width=680,height=520,titlebar=no,directory=no,copyhistory=no,toolbar=no,location=no,Internet Explorer 7.location=no,resizable=no,scrollbars=no,menubar=no,status=no");void(0);'
	    },
        chart: {
        ...
        series: [{
            ...
            name: 'Issues',
            data: [
				{name: 'Critical', y: <%=info.getCritical()%>, sliced: true, selected: true},
				['Major', <%=info.getMajor()%>],
				['Minor', <%=info.getMinor()%>]
            ],
        	animation: {
...
Highcharts.theme = {
   colors: ['#ED561B', '#50B432', '#DDDF00', '#20ADEA', '#64E572', '#FF9655', '#FFF263', '#6AF9C4'],
   chart: {
      backgroundColor: {
