...
public class DBTest {
	...
	String backupDB = "gerrit_infra_test";
	String originDB = "gerrit_swp"; 
	...
	{      
		...
		try {
			stmt2.executeUpdate(sqlQuery);
                        ...
                        writeLog("ERROR : executeUpdate1 TABLE error... T.T");
                        System.out.println("ERROR : executeUpdate1 TABLE error... T.T");
                        e2.printStackTrace();
                        DBMSdisconnect();
                        System.exit(1);
                }

		writeLog("DROP TABLE IF EXISTS "+backupDB+"."+table_name);

		try {
                        stmt2.executeUpdate(sqlQueryCreate);
                } catch (SQLException e2) {
                        // TODO Auto-generated catch block
                                writeLog("ERROR : executeUpdate2 TABLE error... T.T");
                        System.out.println("ERROR : executeUpdate2 TABLE error... T.T");
                        e2.printStackTrace();
                        DBMSdisconnect();
                        System.exit(1);
                }

		writeLog("CREATE TABLE "+backupDB+"."+table_name+" SELECT * FROM "+originDB+"."+table_name);
...
//                      System.out.println("#########executeQuery success!!#########");
        ...
        public void sqlQueryEnter(int n)
        {
                if(n==1){
			table_name = "changes";
                        sqlQuery = "DROP TABLE IF EXISTS "+backupDB+"."+table_name;
			sqlQueryCreate = "CREATE TABLE "+backupDB+"."+table_name+ " SELECT * FROM "+originDB+"."+table_name; 
