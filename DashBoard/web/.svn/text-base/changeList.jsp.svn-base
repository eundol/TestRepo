...
<% 
ArrayList<ChangeInfo> List = (ArrayList<ChangeInfo>)request.getAttribute("List");
...
%>
			  <%= entity.getSubject().substring(0, 77)%>...
<%
			}else{
%>
			  <%= entity.getSubject()%>
...
%>			  
			...
			&nbsp;<font style="font-style:normal;font-variant:normal;font-family:monospace;text-decoration:none;" title="Send to email">
			<a href = "mailto:<%=entity.getPreferredEmail()%>">(<%=entity.getFull_name()%>)</a></font>
		</div>
<%
		}
	}	
