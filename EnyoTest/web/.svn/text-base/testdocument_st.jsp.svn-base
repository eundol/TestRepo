...
if(request.getRequestURL().toString().contains("10.177.234.48") || request.getRequestURL().toString().contains("profilevu.lge.com")){
	documentFilePath = "/home/seadmin/workspace/EnyoTest/web/document_st/";
}else{
	documentFilePath = "C:/eun/workspace/EnyoTest/web/document_st/";
}
	File f = new File(documentFilePath);
	File[] fileList = f.listFiles();
	ArrayList<File> fileListSorted = new ArrayList<File>();
	ArrayList<File> fileListSorted2 = new ArrayList<File>();
	int fileCount = 0;
	for(int i = 0 ; i < fileList.length ; i++){
		if(!fileList[i].getName().equals(".svn") && (fileList[i].getName().endsWith("xlsx") || fileList[i].getName().endsWith("xls"))){
			fileCount++;
	...
	for(int i = 0 ; i < fileList.length ; i++){
		if(!fileList[i].getName().equals(".svn") && !(fileList[i].getName().endsWith("xlsx") || fileList[i].getName().endsWith("xls"))){
			fileCount++;
			fileListSorted2.add(fileList[i]);
		}
	}
	for(int i = 0 ; i < fileListSorted2.size()-1 ; i++){
		for(int j = i+1 ; j < fileListSorted2.size() ; j++){
			if((fileListSorted2.get(j).getName()).compareTo(fileListSorted2.get(i).getName()) < 0){
				File temp = fileListSorted2.get(i);
				fileListSorted2.set(i, fileListSorted2.get(j));
				fileListSorted2.set(j, temp);
			}
		}
	}
...
%>
<body>
<%=fileCount%> Scenario Test documents
...
<%}} %>
<%for(int i = 0 ; i < fileListSorted2.size() ; i++){ 
	if(!fileListSorted2.get(i).getName().equals(".svn")){%>
	<%-- <div style="margin-top: 5px;"><a href="download?filePath=<%=contextUrl%>document/<%=fileList[i].getName()%>&fileName=<%=fileList[i].getName()%>"> --%>
	<div style="margin-top: 7px;"><a href="download?filePath=<%=documentFilePath%><%=fileListSorted2.get(i).getName()%>&fileName=<%=fileListSorted2.get(i).getName()%>" class="link">
	<%=fileListSorted2.get(i).getName()%></a></div>
<%}} %>
<br><br>
</body>
</html>