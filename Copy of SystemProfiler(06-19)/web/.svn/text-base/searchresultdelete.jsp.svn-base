...
<% ArrayList<CpuSearchEntity> CpuSearchResult = (ArrayList<CpuSearchEntity>)session.getAttribute("CpuSearchResult");%>
...
<script type="text/javascript">
	function goCpuTab(id) {
		var domain = document.domain;
		opener.location.href= "http://" + domain + ":8080/report/cpu?id=" + id;
		window.close();
	}
...
<form action="searchresultdelete" method="post" id="deleteform">
<div style="width: 96%; margin: 0 auto 0 auto; margin-top: 14px; overflow-y:auto; height: 400px; overflow-x:hidden;">
	<table style="width: 100%;">
		...
		<tr style="background-color: #F4F4F4;cursor: pointer;" 
			...
			>
			<td>
			 	<input type="checkbox" name="deletecheckid" value="${entity.id}">
			</td>
...
<script type="text/javascript">
	function deleteform(){
		var deletecheckids = document.getElementsByName("deletecheckid");
		var cnt = 0;
		for(var i = 0 ; i < deletecheckids.length ; i++){
			if(deletecheckids[i].checked == true){cnt++;};
		}
		if(cnt != 0){
			var input = prompt("Enter the administrator password","");
			if(input != null && name !=""){
				if(input == "admin"){
	...
	function selectAll(){
		if(flag == false){
			document.getElementById("allSelect").setAttribute("class","allSelected");
			var deletecheckids = document.getElementsByName("deletecheckid");
			for(var i = 0 ; i < deletecheckids.length ; i++){
				deletecheckids[i].checked = true;
			}	
		flag = true;
		return;
		}
		if(flag == true){
			document.getElementById("allSelect").setAttribute("class","allSelect");
			var deletecheckids = document.getElementsByName("deletecheckid");
			for(var i = 0 ; i < deletecheckids.length ; i++){
				deletecheckids[i].checked = false;
			}	
		flag = false;
		return;
		}
	...
	<div style="margin-top : 25px; text-align: center;">
		<a href="javascript:deleteform()">Delete</a>&nbsp;&nbsp;&nbsp;
		<a href="javascript:selectAll()" id="allSelect" class="allSelect">Select all</a>&nbsp;&nbsp;&nbsp;
		<a href="searchresult.jsp">Cancel</a>
	</div>
</c:if>
