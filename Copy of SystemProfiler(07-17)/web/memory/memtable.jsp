...
<%if(memoryInfo != null && !memoryInfo.isEmpty()){ %>
	...
	<table style="width: 100%; height: 100%; table-layout: fixed;">
		...
		<tbody id="tbody">
			...
			int sequence = 0;
			int totalInt = memoryInfo.get(0).getTotal();
			int maxUsed = memoryInfo.get(0).getUsed();
			...
			%>
				<tr class="basic" id="tr<%=sequence%>">
					...
					<td style="text-align: center; cursor: default;" title="<%= df.format(entity.getUsed()/1024) %> KB">
						<%= ((double)(Math.round(((double)entity.getUsed() * 1000  / (double)totalInt))) / (double)10) %>%
					</td>
					<td style="text-align: center; cursor: default;" title="<%= df.format(entity.getBuffers()/1024) %> KB">
						<%= ((double)(Math.round(((double)entity.getBuffers() * 1000  / (double)totalInt))) / (double)10) %>%
					</td>
					<td style="text-align: center; cursor: default;" title="<%= df.format(entity.getCached()/1024) %> KB">
						<%= ((double)(Math.round(((double)entity.getCached() * 1000  / (double)totalInt))) / (double)10) %>%
					</td>
					<td style="text-align: center; cursor: default;" title="<%= df.format(entity.getFree()/1024) %> KB">
						<%= ((double)(Math.round(((double)entity.getFree() * 1000  / (double)totalInt))) / (double)10) %>%
					</td>
				</tr>
			<%} %>
		</tbody>
...
font-size: 16px; position: fixed; top:6px; width:100%; text-align:center; color: #B22222; ">Memory Infomation (per second)</div>
...
&nbsp;&nbsp;(<%=memoryInfo.get(memoryInfo.size()-1).getSequence()%> Seconds Measured)
</div>

<div style="position: fixed; top:34px; width:100%; text-align:right; right: 13px; font-size: 11px; cursor: default; color: blue;">

<span onclick=""
style="font-weight: bold; text-decoration: underline; font-size: 11px; cursor: default;" id="percentage">%</span>
/
<span onclick="javascript:unitChange('byte')"
style="font-weight: normal; text-decoration: none; font-size: 11px; cursor: pointer;" id="byte">Byte</span>
