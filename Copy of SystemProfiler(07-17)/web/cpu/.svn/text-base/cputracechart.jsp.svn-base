...
<%
	...
	if(created != null && !created.equals("") && !created.equals("null")){
		...
		for(int inx = 0, size = cpuTraceInfo.size(); inx < size ; inx++){
		...
		if(cpuTraceInfo.size() < 8){
			maxZoom = "2250";
		}
		
		if(!maxInfo.getUsed().equals("0") && !maxInfo.getUsed().equals("0.00")){
			data += profileUtils.makeTraceGraphString(cpuTraceInfo, "Used", created)+",";
		}
		if(!maxInfo.getUser().equals("0") && !maxInfo.getUser().equals("0.00")){
			data += profileUtils.makeTraceGraphString(cpuTraceInfo, "User", created)+",";
		}
		if(!maxInfo.getSystem().equals("0") && !maxInfo.getSystem().equals("0.00")){
			data += profileUtils.makeTraceGraphString(cpuTraceInfo, "System", created)+",";
		}
		if(!maxInfo.getIowait().equals("0") && !maxInfo.getIowait().equals("0.00")){
			data += profileUtils.makeTraceGraphString(cpuTraceInfo, "Iowait", created)+",";
		}
		if(!maxInfo.getNice().equals("0") && !maxInfo.getNice().equals("0.00")){
			data += profileUtils.makeTraceGraphString(cpuTraceInfo, "Nice", created)+",";
		}
		if(!maxInfo.getSteal().equals("0") && !maxInfo.getSteal().equals("0.00")){
			data += profileUtils.makeTraceGraphString(cpuTraceInfo, "Steal", created)+",";
		}
...
$(function () {
       $('#container6').highcharts({
            ...
            title: {
                text: 'Trace Infomation <span style="font-size:12px;">(<%=created.substring(11)%> ~ <%=endTime.substring(11)%>)</span>'
            },
            subtitle: {
                text: 'The Used Rate is the highest at <span style="text-decoration: underline;"><%=maxTime.substring(11)%></span> - Total <%=cpuTraceInfo.size()%> Seconds Measured'
            },
            xAxis: {
                type: 'datetime',
...
Highcharts.theme = {
   colors: ['#ED561B', '#058DC7','#50B432', '#DDDF00', '#FF9655', '#24CBE5', '#ACAE00'],
    chart: {
      backgroundColor: {
