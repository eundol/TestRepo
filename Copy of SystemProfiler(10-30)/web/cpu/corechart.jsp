...
<%
	...
	String[] barColor = {"#24CBE5","#ED561B"};  
/* 	String[] pieColor = {"#ED561B","#24CBE5","#345BF8","#CF720A","#6F3D9C","#008040","#FF0080","#3FB4C0"}; */
	...
	Double[] totalPerCore = new Double[2];
	...
	for(int i = 0 ; i < list.size() ; i++){
		...
		if(entity.getCommand().equals("swapper")){
			...
			totalPerCore[1] += entityOverheadNum;
		}else{
			usagePerCore[cpuCoreValue] += entityOverheadNum;
			totalPerCore[0] += entityOverheadNum;
		}
	}
	...
	pieChartP = pieChartP + "{name : '"+ "Idle" + "', y : " + df.format(totalPerCore[1]) 
			 + ", color : '" + pieColor[1] + "'},";
	pieChartP = pieChartP + "{name : '"+ "Usage" + "', y : " + df.format(totalPerCore[0]) 
			 + ", color : '" + pieColor[0] + "'}";
	
	/* for(int i = maxCpuNum ; i >= 0 ; i--){
		pieChartP = pieChartP + "{name : '"+ "Usage" + "', y : " + df.format(totalPerCore[i]) 
				 + ", color : '" + pieColor[i] + "'}";
		if(i != 0){
			pieChartP = pieChartP + ",";
		}
