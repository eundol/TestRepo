...
<%
	...
	if(created != null && !created.equals("") && !created.equals("null") && !memoryInfo.isEmpty()){
		...
		MemEntity entity = null;
			...
			if(memoryInfo.get(memoryInfo.size()-1).getSequence() > 20){
				labels ="Highcharts.numberFormat(this.y/1024/1024, '', ',',',')";
			}
			if(memoryInfo.get(memoryInfo.size()-1).getSequence() > 34){
				labels ="";
			}
			
		if(memoryInfo.size() < 8){
			maxZoom = "2250";
		}
		
		duration = (int)memoryInfo.get(memoryInfo.size()-1).getSequence();
...
$(function () {
       $('#container11').highcharts({
            ...
            subtitle: {
                text: 'Max Available Memory : <%=total%> (MB) / Measured for <%=duration%> Seconds'
            },
            xAxis: {
                type: 'datetime',
...
Highcharts.theme = {
   ...
   yAxis: {
      minorTickInterval: 104857600,
      tickInterval: 209715200,
      lineColor: '#000',
      lineWidth: 1,
      tickWidth: 1,
      tickColor: '#000',
...
<body>
<div id="container11" style="width: 98%; height: 340px; margin: 0 auto"></div>
<div style="position: fixed; top: 313px; right: 2%">
