...
$(function () {
    ...
    $('#highusage').highcharts({
        ...
        title: {
            text: 'Highest Usage Increases',
             style:{
                color : '#B22222'
            }
        },
        subtitle:{
			text: 'The top 5 processes that have the highest increase in memory consumption' 
        },
        xAxis: {
	        lineColor: '#000',
	        tickColor: '#000',
        ...
        yAxis: {
            ...
            labels: {
               ...
               formatter : function() {
            	   		if(<%=smemInfo.get(0).getPssDiff()%> > 7000){
            	   			var s = Highcharts.numberFormat(this.value/1024, '', ',',',') + '<span style="font-size:11px; font-weight:normal;">MB</span>';
            	   		}else{
	   						var s = Highcharts.numberFormat(this.value, '', ',',',') + '<span style="font-size:11px; font-weight:normal;">KB</span>';
            	   		}
	   					return s;
				}
            }
        ...
        plotOptions: {
            bar: {
                dataLabels: {
                    ...
                    formatter : function() {
                		if(this.y >= 1024){
                			var s = '<span style="color:red ; font-size:10px; font-weight:bold;">+'+ Highcharts.numberFormat(this.y, '', ',',',') + '</span><span style="color:#ED561B; font-size:9px; font-weight:normal;"> KB</span>';		
                		}else{
        ...
        series: [{
        	...
        	colorByPoint : true,
            data: [
                   {y : <%=smemInfo.get(0).getPssDiff()%>}, 
                   {y : <%=smemInfo.get(1).getPssDiff()%>}, 
                   {y : <%=smemInfo.get(2).getPssDiff()%>}, 
                   {y : <%=smemInfo.get(3).getPssDiff()%>}, 
                   {y : <%=smemInfo.get(4).getPssDiff()%>}
                   ],
            animation: {
                   duration: 2850
                   }
