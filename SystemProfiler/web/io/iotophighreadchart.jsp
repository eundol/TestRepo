...
<%
	ArrayList<IotopEntity> iotopInfo = (ArrayList<IotopEntity>)request.getAttribute("iotopInfo");
	String id = (String)request.getAttribute("id");
	int maxSequence = (Integer)request.getAttribute("maxSequence");
	IotopEntity entity = new IotopEntity();
	
	boolean flagTemp = true;
	
	if(iotopInfo == null || iotopInfo.size() == 0){
		flagTemp = false;		
	}
	
	if(!flagTemp){
		entity.setCommandTrimed("pv");
		entity.setRead(0);
		entity.setWrite(0);
		iotopInfo = new ArrayList<IotopEntity>();
		for(int inx = 0 ; inx < 5 ; inx++){
			iotopInfo.add(entity);
		}	
	}
	
	for(int inx = 0 ; inx < iotopInfo.size() ; inx++){
		if(iotopInfo.get(inx).getCommand().indexOf(" ") > 0){
			iotopInfo.get(inx).setCommandTrimed(iotopInfo.get(inx).getCommand().substring(0, iotopInfo.get(inx).getCommand().indexOf(" ")));
		}else{
			iotopInfo.get(inx).setCommandTrimed(iotopInfo.get(inx).getCommand());
		}
	}
...
$(function () {
    ...
    $('#highusage').highcharts({
        ...
        xAxis: {
            ...
            categories: [
            			 '<%=iotopInfo.get(0).getCommandTrimed()%>',
            			 '<%=iotopInfo.get(1).getCommandTrimed()%>',
            			 '<%=iotopInfo.get(2).getCommandTrimed()%>',
            			 '<%=iotopInfo.get(3).getCommandTrimed()%>',
            			 '<%=iotopInfo.get(4).getCommandTrimed()%>'
                         ],
            title: {
                text: null
            }
        ...
        series: [{
        	...
        	colorByPoint : true,
            data: [
                   {y : <%=iotopInfo.get(0).getRead()%>}, 
                   {y : <%=iotopInfo.get(1).getRead()%>}, 
                   {y : <%=iotopInfo.get(2).getRead()%>}, 
                   {y : <%=iotopInfo.get(3).getRead()%>}, 
                   {y : <%=iotopInfo.get(4).getRead()%>}
                   ],
            animation: {
                   duration: 2850
                   }
