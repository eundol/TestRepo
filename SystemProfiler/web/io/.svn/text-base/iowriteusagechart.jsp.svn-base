...
<%
	...
	ArrayList<IotopEntity> iotopInfoArrangedByWrite = (ArrayList<IotopEntity>)request.getAttribute("iotopInfoArrangedByWrite");
	...
	if(created != null && !created.equals("") && !created.equals("null")){
		...
		if(iotopInfoArrangedByWrite.size() > 0){
	/* 		if(!maxInfo.getUsed().equals("0") && !maxInfo.getUsed().equals("0.00")){
			...
			for(int inx = 0, size = iotopCommandOrder.size() ; inx < size ; inx++){
				data += profileUtils.makeTraceGraphStringIO(iotopInfoArrangedByWrite, iotopCommandOrder.get(inx), created, overThirty, "write") +",";
			}
...
$(function () {
       $('#containerWrite').highcharts({
            ...
            yAxis: {
            	title: {
                    text: 'I/O Write (accumulated)'
                },
                endOnTick : true,
                min : 0,
                labels: {
	                formatter: function() {
	                    return  Highcharts.numberFormat(this.value, 0,'.') + 'KB';
	           		 }
                }
            },
            tooltip: {
       ...
       var chart = $('#containerWrite').highcharts();
...
<body>
<div id="containerWrite" style="width: 100%; height: 320px; margin: 0 auto"></div>
