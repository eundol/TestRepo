...
select origin2.team,
	    sum(origin2.Commited) "Commited",
	    sum(origin2.Merged) "Merged",
	    concat(round((coalesce(sum(origin2.Merged),0) / sum(origin2.Commited)) * 100),'%') "Merged 비율",
	    sum(origin2.Abandoned) "Abandoned",
	    concat(round((coalesce(sum(origin2.Abandoned),0) / sum(origin2.Commited)) * 100),'%') "Abandoned 비율",
	    sum(origin2.Commited) - coalesce(sum(origin2.Merged),0) - coalesce(sum(origin2.Abandoned),0) "Not Reviewed",
	    sum(origin2.개발자수) "개발자수",
	    min(origin2.최초create일) "최초create일",
		 max(origin2.최근create일) "최근create일",
		 concat('W',@week) as "주차"
from
...
join
	...
	group by c.dest_project_name) origin on (mp.repository = origin.dest_project_name)
left join 
   (select p9.project_name,
	coalesce(count(distinct(c9.owner_account_id)),0) "참여개발자" from changes c9
	left join master_project p9 on (c9.dest_project_name = p9.repository)
...
group by mp.project_name) origin2
group by origin2.team;