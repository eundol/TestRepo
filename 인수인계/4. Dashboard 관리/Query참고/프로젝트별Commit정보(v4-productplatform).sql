select output.project "PJT name",
	    output.developers "Push한 개발자수",
	    output.persons "전체 개발자수",
	    output.참여율,
	    output.Merge,
	    output.Abandon,
	    output.Open,
	    output.Total,
	    output.push "전주대비 Push수"
from
(select md.project,
		 a.developers,
		 md.persons,
...
left join
   (select mp1.project_name, 
	...
	group by mp1.project_name) b on (a.project_name = b.project_name)) output
where output.team like 'ProductPlatform';